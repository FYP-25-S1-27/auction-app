...
export async function deleteCategory({
  ...
  const condition =
    mainCategory === null
      ? and(
          isNull(listingCategory.parent),
          eq(listingCategory.name, subCategory)
        )
      : and(
          eq(listingCategory.parent, mainCategory),
          eq(listingCategory.name, subCategory)
        );
...
export async function createCategory(
  ...
  if (existing.length > 0) {
    return {
      success: false,
      message:
        "This subcategory already exists under the selected main category.",
    };
  }
